...
def draw_bounding_box_on_image_array(current_frame_number, image,
  ...
  """
  image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
  is_vehicle_detected, csv_line, update_csv = draw_bounding_box_on_image(current_frame_number,image_pil, ymin, xmin, ymax, xmax, chooseColor, color,
...
def draw_bounding_box_on_image(current_frame_number,image,
  ...
  print(predicted_color, chooseColor)
  if predicted_color == chooseColor:
